...
namespace dnn
{
    ...
    public:
        ...
        {
            return layers_.empty() ? 0 : layers_.front().in_size;
        }

        std::size_t output_size() const
        {
            return layers_.empty() ? 0 : layers_.back().out_size;
        }
        ...
        std::vector<double> predict(const std::vector<double> &input) const
        {
            ...
            std::vector<double> z(layers_[0].out_size);
            std::vector<double> next_a(layers_[0].out_size);
        ...
        // Simple SGD training with MSE loss
